...
const catCls = (c='') => ({
  ...
  seyahat:'bc-seyahat', egitim:'bc-egitim',
}[c.toLowerCase()] ?? 'bc-other')

const accCls = a => ({'mega':'bc-mega','makro':'bc-makro','mikro':'bc-mikro'}[a] ?? 'bc-mikro')
const mlCls  = m => ({'uygun':'ml-u','orta':'ml-o','uygun_degil':'ml-x','uygunsuz':'ml-x'}[m] ?? 'ml-o')
const mlText = m => ({
  uygun: 'Uygun',
  orta: 'Orta',
  uygun_degil: 'Uygun Degil',
  uygunsuz: 'Uygun Degil',
  bilinmiyor: 'Model Yok',
}[m] ?? 'Model Yok')
const mlExplain = m => ({
  uygun: 'XGBoost modeli kampanya, kategori ve skor featurelarini birlikte olumlu degerlendirdi.',
  orta: 'Model sinyali kararsiz; insan onayi veya ek marka kriteri onerilir.',
  uygun_degil: 'Model feature kombinasyonunu riskli/eslesme disi olarak isaretledi; siralama puani dusuruldu.',
  uygunsuz: 'Model feature kombinasyonunu riskli/eslesme disi olarak isaretledi; siralama puani dusuruldu.',
  bilinmiyor: 'Model dosyasi veya feature semasi hazir olmadigi icin tahmin uretilmedi.',
}[m] ?? 'Model dosyasi veya feature semasi hazir olmadigi icin tahmin uretilmedi.')
...
const SCORE_INFO = {
  SFS: {
    title: 'İçerik Uyumu',
    desc: 'Marka metniyle paylaşım dilinin anlamsal yakınlığı',
  },
  NFS: {
    title: 'Performans',
    desc: 'Etkileşim, takipçi ve paylaşım ritmi sinyali',
  },
  CFS: {
    title: 'Kampanya Uyumu',
    desc: 'Bu kampanya tipine benzer içeriklerdeki uygunluk',
  },
  BAS: {
    title: 'Marka Uyumu',
    desc: 'Semantik uyum, kalite ve risk sinyallerinin özeti',
  },
}
...
export default function InfluencerCard({ rank, data }) {
  const {
    ...
    NFS, sfs, cfs, campaign_bas, raw_final_score, ai_adjustment, final_score,
    positive_ratio, fake_followers_risk,
    follower_count, avg_engagement_rate, ml_label, data_source,
  } = data

  const risk = fake_followers_risk ?? 0
  const hasAiAdjustment = ai_adjustment != null && Number(ai_adjustment) !== 0
      ...
      <div className="icard-body">
        ...
        <div className="decision-strip">
          ...
          <div>
            ...
            <strong className={risk > 40 ? 'risk-high' : risk > 15 ? 'risk-mid' : 'risk-low'}>{riskLevel(risk)}</strong>
          </div>
        </div>
        ...
        <div className={`xai-panel ${mlCls(ml_label)}`}>
          ...
          <div className="xai-score-line">
            ...
            <span className={hasAiAdjustment ? 'xai-penalty' : 'xai-neutral'}>
              AI Etkisi: {ai_adjustment!=null?Number(ai_adjustment).toFixed(1):'0.0'}
            </span>
          </div>
        </div>

        {/* user-friendly score grid */}
        ...
        <div className="stats-row">
          ...
          <StatCell
            Icon={ActivityIcon}
            label="Etkileşim"
            value={avg_engagement_rate!=null?`%${avg_engagement_rate.toFixed(1)}`:'—'}
          ...
          <StatCell
            Icon={TrendIcon}
            label="Pozitif"
            value={positive_ratio!=null?`%${positive_ratio.toFixed(0)}`:'—'}
